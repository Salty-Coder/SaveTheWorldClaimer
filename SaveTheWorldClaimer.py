...
try:
    ...
    print(f"The program will now try to install the missing modules.\n")
    subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'requests', 'configparser', 'datetime'])
...
if not os.path.exists(configPath):
    ...
    if bStartSetup == "1":
        ...
        iDiscord_Webhook = validInput(getString("config.setup.discordwebhook"), boolValues)
    ...
    else: iLanguage, iItemsLanguage, iSpend_Research_Points, iOpen_Free_Llamas, iRecycle_Weapons, iRecycle_Traps, iRetire_Survivors, iRetire_Defenders, iRetire_Heroes, iSkip_Tutorial, iLoop_Time, iShow_Date_Time, iDiscord_Webhook, iDiscord_Webhook_URL, iDiscord_Webhook_Type = ["en", "en", "lowest", "true", "uncommon", "uncommon", "rare", "rare", "uncommon", "false", 0, "false", "false", "", "full"]           
    with open(configPath, "w") as configFile: configFile.write(getString("config.configfile").format(', '.join(langValues), iLanguage, ', '.join(itemLangValues), iItemsLanguage, iSpend_Research_Points, iOpen_Free_Llamas, iSkip_Tutorial, iRecycle_Weapons, iRecycle_Traps, iRetire_Survivors, iRetire_Defenders, iRetire_Heroes, iLoop_Time, iShow_Date_Time, iDiscord_Webhook, iDiscord_Webhook_URL, iDiscord_Webhook_Type, configVersion))
    ...
    for account in authJson:
            ...
            if authType == "token":
                ...
                if expirationDate < datetime.now().isoformat():
                    if bDiscordWebhookURL:
                        try:
                            msg = getString("main.auth.tokenexpired").format(displayName)
                            webhook(bDiscordWebhookURL, msg)
                        except:
                            print("Error sending Discord Webhook! Maybe invalid URL?")
                    customError(getString("main.auth.tokenexpired").format(displayName))
            if authType == "device":
        ...
        if authType == "token": # Shoutout to BayGamerYT for telling me about this login method.
            ...
            if "errorMessage" in reqRefreshToken:
                if bDiscordWebhookURL:
                    webhook(bDiscordWebhookURL, getString("main.login.token.error").format(displayName))
                customError(getString("main.login.token.error").format(displayName))
